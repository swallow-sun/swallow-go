...
- Go 1.25 + Hertz（HTTP 框架）+ GORM（ORM）+ PostgreSQL
...
PostgreSQL 由 `../infra/compose.yaml` 持久化；Go 容器映射 HTTP `8888` 和 TTS gRPC `9881` 端口。

```bash
# 首次使用：创建本地配置并填写 owner_token、LLM/ASR/TTS 所需密钥
cp config.toml config.local.toml

# 先启动 PostgreSQL
docker compose -f ../infra/compose.yaml --profile core up -d postgres
...
# 查看状态和日志
docker compose ps
docker compose logs -f swallow-go

# 停止服务（不会删除数据卷）
docker compose down
```

`config.local.toml` 以只读方式挂载，不会写入镜像。Go 直接在宿主机运行时 DSN 使用 `127.0.0.1:5432`；Go 在容器运行时应使用 `host.docker.internal:5432`。数据库数据保存在 infra 的 `postgres-data` 卷中。
...
│   ├── config/          # TOML 配置加载
│   ├── data/            # 数据层（GORM + PostgreSQL，Repository 接口）
│   ├── debug/           # pprof 调试服务
...
- **data**：Repository 接口 + PostgreSQL 实现
...
模型用量看板接口已经迁移到 `swallow-analytics`：

```text
GET /api/v1/dashboard/model-usage?from=YYYY-MM-DD&to=YYYY-MM-DD
```

该接口只查询 Doris；`swallow-go` 不再提供看板路由，也不再向 PostgreSQL 的日聚合表写数据。

## 数据库

PostgreSQL 版本化迁移（`script/migrations/`）：
