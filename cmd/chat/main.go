...
func run() (runErr error) {
	...
	userName := flag.String("user", "owner", "username (default: owner)")
	...
	if err != nil {
		return fmt.Errorf("user init failed: %w", err)
	}
	...
	fmt.Printf("Swallow CLI started (user: %s, session: %s)\n", user.Name, sessionID[:8])
	fmt.Println("Enter a message to start chatting (type quit to exit):")
	...
	for {
		...
		if err != nil {
			...
			logger.Debug("CLI input ended", zap.Error(err))
			return nil // EOF,正常退出
		}
		...
		for {
			...
			if err != nil {
				// 读取出错,打日志,标记失败,跳出循环
				logger.Error("stream read failed", zap.Error(err), zap.String("trace_id", traceID))
				fmt.Printf("\n[read error: %v]\n", err)
				success = false
				break
			}
		...
		logger.Info("LLM stream call completed",
			zap.String("trace_id", traceID),
